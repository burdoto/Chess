...
namespace ChessAPI
{
    ...
    public class ChessGame
    {
        ...
        {
            return new(Path.Combine(Path.Combine(basePath, "Assets/"),
                $"p{(plr == Player.PlayerOne ? '1' : '2')}-{fig}.bmp"));
        }
        ...
        private Vector2 CalcRelative(Vector2 rel)
        {
            return rel + (ActivePosition ?? Vector2.Zero);
        }

        private Vector2 PlayerMirror(Vector2 rel)
        {
            return ActivePlayer == Player.PlayerOne ? -rel : rel;
        }

        private static readonly Vector2 ab = new(1, -1);

        private Vector2[] MirrorDiagonal(Vector2 rel)
        {
            return new[] { rel * Vector2.One, rel * ab, rel * -ab, rel * -Vector2.One };
        }


        private Vector2[] MirrorRadial(Vector2 rel)
        {
            return new[]
            {
